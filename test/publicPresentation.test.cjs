...
test("repository overview uses the public 2K presentation assets", () => {
  ...
  assert.match(readme, /Wayfinder-Alpha-0\.3\.13-macOS-aarch64\.dmg/);
  assert.match(readme, /Wayfinder-Alpha-0\.3\.13-macOS-x86_64\.dmg/);
  assert.match(readme, /Wayfinder-Alpha-0\.3\.13-Windows-x86_64\.exe/);
...
test("website metadata states the product category and current platforms", () => {
  ...
  assert.match(html, /"softwareVersion": "0\.3\.13"/);
  assert.match(
...
test("public support and release documents are explicit and current", () => {
  ...
  assert.match(security, /\| 0\.3\.13 \| Yes \|/);
  assert.match(changelog, /## \[0\.3\.13\] - 2026-09-12/);
