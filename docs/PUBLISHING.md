...
Public release: **0.3.5 early access**. Local next version: **0.3.6
...
| macOS app | https://github.com/WBXWHT/wayfinder/releases/tag/alpha-v0.3.5 | Apple Silicon and Intel DMGs |
...
5. Run `.github/workflows/release-macos-alpha.yml` for the zero-cost Alpha, or
   `.github/workflows/release-macos-companion.yml` when signing credentials are
   available.
6. Verify both architecture DMGs and `SHA256SUMS`, then publish the draft
   GitHub Release.
7. Update `website/releases.json` only after both DMG URLs return successfully.
